...
try {
    const{id,name}=req.body; // nhap id,name
    const sinhvien1=new sinhvien({id,name}); // tao doi tuong sv1 voi gia tri nhap vao
    await sinhvien1.save();// luu vao csdl
    res.status(201).json(sinhvien1);// tra ve ket qua cho nguoi dung biet
    console.log(sinhvien1);
} catch (err) {
...
try {
    ...
    const {name,id}=req.body; // lay noi dung nguoi dung nhap
    // thuc hien update
    const updateSinhvien=await sinhvien.findByIdAndDelete(_id,{id,name},{new:true});
    res.json(updateSinhvien); // tra ket qua cho nguoi dung
